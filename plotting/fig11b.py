...
# different objective functions for rank estimation: isoTNS tensor
def fig11b(data_folder, save_folder):
    l = 8
    r = 8
    b = 60
    c = 12
    ...
    for i, disentangler in enumerate(disentanglers):
        ...
        if i == 0:
            plt.plot(range(l*c-1), get_trunc_err(test['sv_before'])[:-1], c=color_cycle[3], 
                     label='Initial', lw=2, linestyle=l_cycle[5])
        
        plt.plot(range(l*c-1), get_trunc_err(test['sv_after'])[:-1], 
    ...
    for rank_idx, rank in enumerate(ranks):
        ...
        plt.scatter([rank], [test['cost'][-1]], 
                    color=cmap_custom_purple(0.15 + 0.65*rank_idx/2), 
    ...
    plt.axvline([91], c=color_cycle[7], linestyle='--', lw=2, zorder=-2)
    plt.yscale('log')
    ...
    plt.savefig(save_folder + 'fig11b.svg', bbox_inches='tight', dpi=1200, transparent=True)